...
module DatacenterDetector
  class Cache
    ...
    def add(result)
      ...
      record = [cidr, start, finish, result.is_datacenter ? 1 : 0, result.server || result.datacenter || result.asn&.descr, result_hash.to_json]

      @db.transaction do |d|
        d.execute("INSERT INTO #{table} (cidr, start, finish, is_datacenter, name, response, retreived_at, created_at)  VALUES (?, ?, ?, ?, ?, ?, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)", record)
      end
    end
    ...
    def get(ip:, force:false)
      a = IPAddr.new(ip)
      resp = if a.ipv6?
        @db.execute('select cidr, name, is_datacenter, retreived_at, created_at, response from ranges6 where start <= ? and finish >= ?', [a.to_i.to_s(16), a.to_i.to_s(16)]).first
      else
        @db.execute('select cidr, name, is_datacenter, retreived_at, created_at, response from ranges where start <= ? and finish >= ?', [a.to_i, a.to_i]).first
      end

      return nil if resp.nil?

      resp = { cidr: resp[0], name: resp[1], is_datacenter: (resp[2] == 1), retreived_at: resp[3], created_at: resp[4], result: resp[5], status: 301}
